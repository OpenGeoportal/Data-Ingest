...
public class LocalDownloadService {
     ...
     * @return true if the file is already on the filesystem (until it's better defined the caching strategy)
     */
    private boolean isFilePresent(final String workspace, final String dataset) {

        final File f = new File(FileNameUtils.getFullPathZipFile(workspace, dataset));
     ...
     * @return true
     */
    private boolean isAValidRequest(final String workspace, final String dataset) {

        /* Find a better way than asking directly for the file
        (some method in org.geotools.data.store.ContentDataStore?) */
        return true;
    }
    ...
    /**
     * This is the method that we use for getting the file.
     * Consider to cache this method.
     *
     * @param workspace the workspace
     * @param dataset the dataset
     ...
     * @throws FileNotFoundException File not found on OGS
     */
    public final void forceGetFile(final String workspace, final String dataset) throws FileNotFoundException {
